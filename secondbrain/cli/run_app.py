...
from . import BaseStableFusionCommand


def run_app_command_factory(args):
    return RunStableFusionAppCommand(
        args.output,
        args.share,
        args.port,
        args.host,
...
class RunStableFusionAppCommand(BaseStableFusionCommand):
    @staticmethod
    def register_subcommand(parser: ArgumentParser):
        run_app_parser = parser.add_parser(
            "app",
            description="✨ Run stablefusion app",
        )
    ...
    def run(self):
        # from ..app import stablefusion

        # print(self.share)
        # app = stablefusion(self.model, self.output).app()
